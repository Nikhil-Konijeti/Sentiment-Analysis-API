...
if __name__ == "__main__":
    ...
    minority_class_len=len(df[df['AirLine_Sentiment']=='positive'])
    majority_class_indices=df[df['AirLine_Sentiment']=='negative'].index
    
    random_majority_indices = np.random.choice(majority_class_indices,
                                               minority_class_len, 
                                               replace=False)
    minority_class_indices=df[df['AirLine_Sentiment']=='positive'].index
    
    under_sample_indices = np.concatenate([minority_class_indices,random_majority_indices])
    under_sample = df.loc[under_sample_indices]
    sns.countplot(x='AirLine_Sentiment', data=under_sample)
    
    X = under_sample.iloc[:, 1].values
    y = under_sample.iloc[:, 0].values
